...
def gen_golden_data_simple(m, k, n):
    ...
    K = k
    N = n
    cpu_x1 = torch.randint(34, 35, (M, int(K / 2)), dtype=torch.int8)
    cpu_x2 = torch.randint(34, 35, (N, int(K / 2)), dtype=torch.int8)
    scale_x1 = torch.randint(100, 101, (M, math.ceil(K / 64), 2), dtype=torch.int8)
    scale_x2 = torch.randint(100, 101, (N, math.ceil(K / 64), 2), dtype=torch.int8)
