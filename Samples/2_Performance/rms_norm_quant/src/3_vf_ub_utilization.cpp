...
public:
    ...
    __simd_vf__ inline void ComputeRmsVf(__ubuf__ DATA_TYPE *xInAddr, __ubuf__ float *xAddr, __ubuf__ float *rmsAddr,
        uint16_t vfLoopRNum, uint16_t ubFactor)
    {
    ...
    __aicore__ inline void Compute(int64_t ubFactor)
    {
        ...
        ComputeRmsVf(xInAddr, xAddr, rmsAddr, vfLoopRNum, ubFactor);
        ComputeNormQuantVf(xAddr, gammaAddr, betaAddr, rmsAddr, yAddr, vfLoopRNum, ubFactor);
...
int64_t calcMaxUbFactor(size_t r)
{
     ...
     * UB内存分配公式推导:
     * 与maxUbFactor线性相关的部分:
     *   xInQue:  rAlign * maxUbFactor * sizeof(half) * BUF_NUM = rAlign * maxUbFactor * 2
     *   yOutQue: rAlign * maxUbFactor * sizeof(int8_t) * BUF_NUM = rAlign * maxUbFactor * 1
     *   xBuf:    rAlign * maxUbFactor * sizeof(float) = rAlign * maxUbFactor * 4
     *   rmsBuf:  maxUbFactor * BLOCK_INT8_SIZE = maxUbFactor * 32
     * 与maxUbFactor无关的固定部分:
     *   gammaInQue: rAlign * sizeof(half) = rAlign * 2
     *   betaInQue:  rAlign * sizeof(half) = rAlign * 2
     *   gammaBuf:   rAlign * sizeof(float) = rAlign * 4
     *   betaBuf:    rAlign * sizeof(float) = rAlign * 4
     * 总计: maxUbFactor * (rAlign * 7 + 32) + rAlign * 12 <= UB_SIZE
     * 求解: maxUbFactor <= (UB_SIZE - rAlign * 12) / (rAlign * 7 + 32)
     */

    int64_t rAlign = (r + BLOCK_BYTES - 1) / BLOCK_BYTES * BLOCK_BYTES;
    constexpr int64_t coefPerFactor = 7;             // rAlign系数: 2 + 1 + 4 = 7
    constexpr int64_t rmsBufCoef = BLOCK_INT8_SIZE;  // 32
    constexpr int64_t fixedCoef = 12;                // 固定部分rAlign系数: 2 + 2 + 4 + 4 = 12

    int64_t linearCoef = rAlign * coefPerFactor + rmsBufCoef;  // 与maxUbFactor线性相关的系数
    int64_t fixedSize = rAlign * fixedCoef;                    // 固定部分大小
