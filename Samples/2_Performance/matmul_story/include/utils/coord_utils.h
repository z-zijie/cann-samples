...
public:
    ...
    __aicore__ inline AscendC::Std::tuple<int64_t, int64_t, int64_t, int64_t, int64_t> GetQuantOffset(
        ...
        const AscendC::Std::tuple<int64_t, int64_t, int64_t, int64_t>& loadBalanceParam = {0LL, 0LL, 0LL, 0LL})
    {
        ...
        Get<0>(offset) = mOffset * k;                                                    // 0: idx of A
        Get<1>(offset) = nOffset * k;                                                    // 1: idx of B
        Get<2>(offset) = mOffset * CeilDiv(k, MXFP_DIVISOR_SIZE) * MXFP_MULTI_BASE_SIZE; // 2: idx of scaleA
        Get<3>(offset) = nOffset * CeilDiv(k, MXFP_DIVISOR_SIZE) * MXFP_MULTI_BASE_SIZE; // 3: idx of scaleB
        Get<4>(offset) = mOffset * n + nOffset;                                          // 4: idx of C
        return offset;
    }
