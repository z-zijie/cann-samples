...
__global__ __aicore__ __vector__ void add_kernel(
    __gm__ T *x, __gm__ int32_t *expertIdx,  __gm__ int32_t *workspace, __gm__ int32_t *expandedRowIdx, __gm__ T *expandedX, 
    int64_t needCoreNum, 
    int64_t perCoreElements, // sort
    int64_t perCoreLoops,
    ...
    int64_t n, int64_t k, int64_t cols)
{
    ...
    int64_t totalLength = perCoreElements;

    int64_t kvFactor = 2; // key and value
    int64_t sortNum = (n * k + ONE_REPEAT_SORT_NUM - 1) / ONE_REPEAT_SORT_NUM * ONE_REPEAT_SORT_NUM;
    int64_t buffSize = sortNum * sizeof(int32_t) * kvFactor;
    ...
    int64_t tileLength = (totalLength * sizeof(int32_t) + 32 - 1) / 32 * 32 / sizeof(int32_t);
    expertIdxGm.SetGlobalBuffer(expertIdx, tileLength);
    sortedexpertIdxGm.SetGlobalBuffer(workspace,
                                      (totalLength * sizeof(int32_t) + 32 - 1) / 32 * 32 / sizeof(int32_t));
    ...
    if (blockIdx_ < 1) {
        ...
        // CopyOut
        LocalTensor<int32_t> res = sortDataCopyOutQueue.DeQue<int32_t>();
        DataCopyParams intriParams;
        intriParams.blockCount = 1;
        intriParams.blockLen = totalLength * sizeof(int32_t);
        DataCopyPad(sortedexpertIdxGm, res[0], intriParams);
        DataCopyPad(expandedRowIdxGm, res[sortNum], intriParams);
        sortDataCopyOutQueue.FreeTensor(res);
...
int main()
{
    ...
    add_kernel<float><<<numBlocks, nullptr, stream>>>(xDevice, expertIdxDevice, workspaceDevice, expandedRowIdxDevice, expandedXDevice, 
        needCoreNum,
        perCoreElements, //sort
        perCoreLoops,
        perCorePerLoopElements,
        perCoreLastLoopElements,
        lastCoreElements,
        lastCoreLoops,
        lastCorePerLoopElements,
        lastCoreLastLoopElements,
