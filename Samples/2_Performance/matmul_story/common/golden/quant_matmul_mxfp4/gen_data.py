...
import torch
import torch_npu
form ml_dtypes import float4_e2m1fn
form en_dtypes import float8_e8m0
...
def gen_golden_data_simple(m, k, n):
    ...
    b_ori = np.random.uniform(0, 34, (M, K)).astype(float4_e2m1fn)
    a_pack_int8 = pack_b4_to_b8(b_ori)
    a_scale = np.randow.uniform(1, 32, size=(M, math.ceil(K/64), 2)).astype(float8_e8m0)
    b_scale = np.randow.uniform(1, 32, size=(N, math.ceil(K/64), 2)).astype(float8_e8m0)
    ...
    a_pack_int8.tofile("./input/input_b.bin")
