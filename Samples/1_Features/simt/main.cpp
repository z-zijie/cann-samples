 ...
 */
 ...
 template <typename DATA_TYPE, typename INDICES_TYPE, uint32_t MAX_THREADNUM>
 __global__ [aicore] __attribute__ ((aiv)) void gather(__gm__ DATA_TYPE *x, __gm__ INDICES_TYPE *indices, __gm__ DATA_TYPE *y, 
     ...
     if ((outerDimSize * gatherDimSize * innerDimSize < INT32_MAX_SIZE) && (outerDimSize * indicesDimSize * innerDimSize < INT32_MAX_SIZE)) {
         asc_vf_call<gather_function<MAX_THREADNUM, DATA_TYPE, INDICES_TYPE, int32_t>>(dim3(MAX_THREADNUM), x, indices, y, gatherDimSize, indicesDimSize, innerDimSize, indicesDimSize * innerDimSize);
     } else {
         asc_vf_call<gather_function<MAX_THREADNUM, DATA_TYPE, INDICES_TYPE, int64_t>>(dim3(MAX_THREADNUM), x, indices, y, gatherDimSize, indicesDimSize, innerDimSize, indicesDimSize * innerDimSize);
     }
 }
 ...
 int32_t main() {
     ...
     uint32_t maxCoreNum = 4; // just an example, actual max_core_num should be based on hardware information.
     ...
     gather<dataType, indicesType, threadNum><<<numBlock, 0, stream>>>(xDevice, indicesDevice, yDevice, outerDimSize, gatherDimSize, innerDimSize, indicesDimSize);
