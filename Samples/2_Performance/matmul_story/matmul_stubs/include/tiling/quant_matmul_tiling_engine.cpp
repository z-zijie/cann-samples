...
void QuantMatmulTilingEngine::InitCompileInfo()
{
    ...
    printf("aicNum: %lu, aivNum: %lu, ubSize: %lu, l1Size: %lu, l0aSize: %lu, "
           "l0bSize: %lu, l0cSize: %lu, l2Size: %lu, btSize: %lu\n",
...
void QuantMatmulTilingEngine::CalcBasicBlock()
{
    ...
    runInfo_.baseK = Align(std::min(args_.k, BASIC_BLOCK_SIZE_256), MXFP_DIVISOR_SIZE);
...
void QuantMatmulTilingEngine::CalL1TilingDepthAfullload()
{
    ...
    uint64_t scaleAL1Size = runInfo_.baseM * Align(CeilDiv(args_.k, MX_GROUP_SIZE), MXFP_MULTI_BASE_SIZE);
...
uint64_t QuantMatmulTilingEngine::GetDepthB1AfullLoad(uint64_t leftSize)
{
    ...
    if (baseStepK == 1UL && args_.k > runInfo_.baseK && leftSize > baseSize * refinedStepkb) {
...
uint64_t QuantMatmulTilingEngine::GetScaleFactorBAfullLoad(uint64_t leftSize)
{
    ...
    uint64_t scaleFactorBMaxFromK = args_.k / (runInfo_.stepKb * runInfo_.baseK);
...
void QuantMatmulTilingEngine::CalStepKs()
{
    ...
    runInfo_.stepKa = std::min(runInfo_.stepKa, 4UL); // 限制stepKa最大为4, 防止issue queue阻塞
    runInfo_.stepKb = std::min(runInfo_.stepKb, 4UL); // 限制stepKb最大为4, 防止issue queue阻塞
    ...
    uint64_t baseASize, uint64_t baseBSize, uint64_t baseScaleASize, uint64_t baseScaleBSize)
{
    ...
    // 来自L1 size 的约束
    uint64_t leftL1sie = platformInfo_.l1Size - (runInfo_.depthA1 * baseASize + runInfo_.depthB1 * baseBSize);
...
void QuantMatmulTilingEngine::SetTiling(QuantMatmulTilingData& tilingData)
{
    ...
    tilingData.enableAFullLoad = runInfo_.isAFullLoad;
    tilingData.usedCoreNum = (runInfo_.totalBlockCnt > 1UL || runInfo_.tailBlockCnt == 0UL) ?
    ...
    CalculateNBufferNum4MX(tilingData);
}

void QuantMatmulTilingEngine::CalculateNBufferNum4MX(QuantMatmulTilingData& tilingData)
{
    tilingData.stepK = std::min(runInfo_.stepKa, runInfo_.stepKb);
    uint64_t kL1 = tilingData.stepK * tilingData.baseK;
    uint64_t usedL1Size = GetSizeWithDataTypeFP4(runInfo_.baseN * kL1) * L1_FOUR_BUFFER;
    usedL1Size += runInfo_.baseN * CeilDiv(static_cast<uint64_t>(tilingData.scaleKL1), MX_GROUP_SIZE) * DB_SIZE;
    if (runInfo_.isAFullLoad) {
        uint64_t scaleK = CeilDiv(args_.k, MXFP_DIVISOR_SIZE) * MXFP_MULTI_BASE_SIZE;
        uint64_t kAligned = Align(args_.k, MXFP_DIVISOR_SIZE);
        ...
        usedL1Size += runInfo_.baseM * CeilDiv(static_cast<uint64_t>(tilingData.scaleKL1), MX_GROUP_SIZE) * DB_SIZE;
    }
    tilingData.nBufferNum = usedL1Size < platformInfo_.l1Size ? L1_FOUR_BUFFER : DB_SIZE;
}

void QuantMatmulTilingEngine::PrintTilingData(const QuantMatmulTilingData& tilingData)
{
    printf(">>>>>>>>>> QuantMatmulTilingData <<<<<<<<<<\n");
    printf("usedCoreNum: %u, enableAFullLoad: %u, m: %u, n: %u, k: %u, baseM: %u, baseN: %u, baseK: %u, "
           "scaleKL1: %u, stepK: %u, mTailTile: %u, nTailTile: %u, mBaseTailSplitCnt: %u, nBaseTailSplitCnt: %u, "
           "mTailMain: %u, nTailMain: %u, nBufferNum: %u, dbL0c: %u\n",
           tilingData.usedCoreNum, tilingData.enableAFullLoad, tilingData.m, tilingData.n, tilingData.k,
           tilingData.baseM, tilingData.baseN, tilingData.baseK, tilingData.scaleKL1, tilingData.stepK,
           tilingData.mTailTile, tilingData.nTailTile, tilingData.mBaseTailSplitCnt, tilingData.nBaseTailSplitCnt,
           tilingData.mTailMain, tilingData.nTailMain, tilingData.nBufferNum, tilingData.dbL0c);
}
