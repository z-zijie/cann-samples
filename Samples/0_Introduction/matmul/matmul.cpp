...
int Init(int32_t deviveId, aclrtStream* stream) {
    auto ret = aclInit(nullptr);
    CHECK_RET(ret == ACLSUCCESS, LOG_PRINT("aclInit failed. ERROR: %d\n", ret); return ret);
    ret = aclrtSetDevice(deviveId);
    CHECK_RET(ret == ACLSUCCESS, LOG_PRINT("aclrtSetDevice failed. ERROR: %d\n", ret); return ret);
    ret = aclCreatStream(stream);
    CHECK_RET(ret == ACLSUCCESS, LOG_PRINT("aclCreatStream failed. ERROR: %d\n", ret); return ret);
    return 0;
}
...
namespace golden {
    ...
    void FillRandomData(std::vector<Element>& data, Element min, Element max) {
        ...
        if constexpr (std::is_integral<Element>::value) {
            ...
            for (auto& ele : data) elem = dist(gen);
    ...
    std::vector<uint64_t> Compare(std::vector<Element>& hostOutput, std::vector<Element>& goldenOutput) {
        ...
        for (uint64_t i = 0; i < hostOutput.size(); ++i) {
            ...
            Element exepectValue = goldenOutput[i];
            Element diff = std::fabs(actualValue - exepectValue);
            if (diff > rtol * std:: max(1.0f, std::fabs(exepectValue))) {
...
int main(int argc, char* argv[]) {
    ...
    vector<uint64_t> errorIndices = golden::Compare<float>(hostOutput, goldenOutput);
