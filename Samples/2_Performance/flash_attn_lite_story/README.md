# 【建设中】 Flash Attention Lite：Ascend 950 上的 1 Cube + 2 Vector 协同优化
...
- `D=128`、`Br=Bc=128`；
- `S` 可为任意正整数，`B`、`N` 均可变；末尾不足 128 行时由 Kernel 在片上补齐；
- 一组 `1 AIC + 2 AIV` 的 Mix 核协作；
- Host 侧把 `B×N` 合并为独立序列，再按 Query tile 将任务分配给多个 Mix 核。

### 不支持的功能

- 运行时切换 non-causal 和 causal；
- 非方形 Q/KV 长度、causal offset、滑动窗口和 KV Cache；
- 同一批次中每条序列长度不同的 varlen 输入、Q/K/V Head 数不一致的 GQA/MQA、可变 HeadDim 或 tile 大小；
- Attention bias、Dropout、反向计算和其他网络融合能力。

> **规模范围：** 本样例的功能正确性保证范围为 `B×N×S≤131072`。更大规模不在保证范围内；该范围是样例的验收边界，不是 Host 的参数拦截条件。
...
| 参数 | 含义与约束 |
| --- | --- |
| `dQ`、`dK`、`dV` | 设备侧 BF16 输入，逻辑形状均为 `(B,N,S,128)` |
| `dOut` | 设备侧 BF16 输出，逻辑形状为 `(B,N,S,128)` |
| `batchSize` | Batch 大小 `B`，必须大于 0 |
| `headNum` | Q/K/V 共用的 Head 数 `N`，必须大于 0 |
| `seqLen` | 序列长度 `S`，必须大于 0，无需按 128 对齐 |
...
Kernel 固定发射 causal 模板实例。源码保留 non-causal 模板分支，便于对照两种遍历方式，但公共接口没有运行时开关。

### 版本总览

先用几个源码术语概括版本路线。一个 `task` 负责某个 `(b,n)` 下一个 Query tile 的完整计算；它每访问一个 Key/Value tile，就产生一个 `item`。
...
后文把 V1/V2 的原始写法称为“分步 Vector”：Softmax、BF16 转换和输出更新分开执行。“压缩 Vector”则把能合并的步骤放进同一个 Vector Function，并简化首个 item 的状态初始化。这里的“压缩”指减少执行步骤，不是压缩数据。
...
| [v6](src/v6/README.md) | 首个 C2 前先发射 2 个 C1（`R=2`） | 让新 C1/V1 越过固定分组边界 |
| [v7](src/v7/README.md) | 首个 C2 前先发射 3 个 C1（`R=3`） | 用更多独立工作覆盖 Vector 等待 |
| [v8](src/v8/README.md) | 首个 C2 前先发射 4 个 C1，保留分步 Vector | 判断只增加提前量是否还有收益 |
| [v9](src/v9/README.md) | `R=3`，并缩短 Vector 计算 | 减少每个 V1/V2 的执行时间 |
| [v10](src/v10/README.md) | `R=4`，并缩短 Vector 计算 | 分别比较 Vector 写法和提前量 |
| [v11](src/v11/README.md) | `R=5`，L0C 仍为四槽 | 第五轮提前发射只带来约 0.5% 变化 |

causal 裁剪和核内 Mutex 是 v0～v11 的共同能力，不作为某一版的性能增量。v9 回到 `R=3`，单独试验压缩 Vector，因此 v7～v11 不是一条只增不减的配置链，比较时要先确认两版之间究竟改变了什么。
...
![FALite v0～v11 性能](./images/chart/falite_performance.png)
...
| $n_{\mathrm{tile}}$ | Query 和 Key/Value 的 tile 数 | $\lceil S/128\rceil$ |
| $q_i$、$k_j$ | 第 $i$ 个 Query tile、第 $j$ 个 K/V tile 的有效行数 | $\min(128,S-128i)$、$\min(128,S-128j)$ |
| $i$ | Query tile 下标 | $0,\ldots,n_{\mathrm{tile}}-1$ |
| $j$ | Key/Value tile 下标 | $0,\ldots,n_{\mathrm{tile}}-1$ |
| $r$、$c$ | tile 内的有效 Query 行和 Key 行 | $0\le r<q_i$、$0\le c<k_j$ |
...
$$
\mathbf Q_i\in\mathbb R^{q_i\times D},\qquad
\mathbf K_j,\mathbf V_j\in\mathbb R^{k_j\times D}.
$$

Cube 仍按固定的 $128\times128$ tile 计算。最后一个 tile 不足 128 行时，Kernel 只从 GM 读取有效的 `Q/K/V` 行，在 L1 的其余位置补 0；Softmax 排除补齐的 Key 行，最终也只把有效 Query 行写回 GM。下文公式描述逻辑有效区域，硬件上的补齐不会改变数学结果。
...
FALite 把 Attention 矩阵切成物理大小为 $128\times128$ 的 tile。第 $i$ 个 Query tile 只读取 $j\le i$ 的 Key/Value tile：
...
对角 tile 的 AIC 仍执行完整矩阵乘。AIV 在最大值扫描和指数求和扫描中，把块内上三角以及尾块中补齐的 Key 行都视为 $-\infty$，确保这些位置既不影响最大值，也不会进入指数和。保存给 C2 的 `P` 会把这些位置写成 0。
...
$$
\mathbf R_{i,j}=\mathbf Q_i\mathbf K_j^\top\in\mathbb R^{q_i\times k_j}.
$$
...
对固定的 $i$，从 $j=0$ 开始维护三份 FP32 状态：逐行最大值 $\mathbf m_{i,j}\in\mathbb R^{q_i}$、逐行指数和 $\boldsymbol\ell_{i,j}\in\mathbb R^{q_i}$ 和未归一化输出 $\mathbf O^{\mathrm{acc}}_{i,j}\in\mathbb R^{q_i\times D}$。
...
$$
\boldsymbol\alpha_{i,j}=\exp\left(\mathbf m_{i,j-1}-\mathbf m_{i,j}\right)\in\mathbb R^{q_i}.
$$

本 tile 的指数分子为：

$$
\mathbf P_{i,j}=\exp\left(\mathbf X_{i,j}-\mathbf m_{i,j}[:,\mathrm{None}]\right)\in\mathbb R^{q_i\times k_j}.
$$
...
| 符号 | 含义 | 形状 | 来源或定义 | 所处时刻 |
| --- | --- | --- | --- | --- |
| $\mathbf Q_i$ | 第 $i$ 个 Query tile 的有效行 | $\mathbb R^{q_i\times128}$ | 输入 $\mathbf Q$ 的连续 $q_i$ 行 | 一个 task 开始时 |
| $\mathbf K_j$、$\mathbf V_j$ | 第 $j$ 个 Key/Value tile 的有效行 | $\mathbb R^{k_j\times128}$ | 输入 $\mathbf K$、$\mathbf V$ 的连续 $k_j$ 行 | 一个 item 开始时 |
| $\mathbf R_{i,j}$ | 未缩放的有效分数区域 | $\mathbb R^{q_i\times k_j}$ | $\mathbf Q_i\mathbf K_j^\top$ | C1 后 |
| $\mathbf X_{i,j}$ | 已缩放并应用 causal mask 的有效分数区域 | $\mathbb R^{q_i\times k_j}$ | $\gamma\mathbf R_{i,j}+\mathbf M_{i,j}$ | V1 第一遍扫描 |
| $\mathbf m_{i,j}$ | 处理到第 $j$ 个 K/V tile 后的逐行最大值 | $\mathbb R^{q_i}$ | Online Softmax 递推 | V1 后 |
| $\boldsymbol\alpha_{i,j}$ | 旧输出和旧指数和的缩放系数 | $\mathbb R^{q_i}$ | $\exp(\mathbf m_{i,j-1}-\mathbf m_{i,j})$ | V1 后，V2 使用 |
| $\mathbf P_{i,j}$ | 本 tile 有效区域的指数分子 | $\mathbb R^{q_i\times k_j}$ | $\exp(\mathbf X_{i,j}-\mathbf m_{i,j})$ | V1 后，C2 使用 |
| $\boldsymbol\ell_{i,j}$ | 处理到第 $j$ 个 K/V tile 后的逐行指数和 | $\mathbb R^{q_i}$ | Online Softmax 递推 | V1 后 |
| $\boldsymbol\Delta\mathbf O_{i,j}$ | 本 tile 对输出分子的增量 | $\mathbb R^{q_i\times128}$ | $\mathbf P_{i,j}\mathbf V_j$ | C2 后 |
| $\mathbf O^{\mathrm{acc}}_{i,j}$ | 尚未除以 $\boldsymbol\ell$ 的输出累加 | $\mathbb R^{q_i\times128}$ | V2 递推 | V2 后 |
...
```text
Rows: qRows = min(128, S - 128*i)
      kRows = min(128, S - 128*j)
C1: R_ij^T = K_j Q_i^T
...
End: O_i = OAcc / l[:, None]，只写回 qRows 行
```
...
| AIV0 | Query tile 前 64 个物理行的 V1/V2；最终只写回其中的有效行 | 自己的 $\mathbf m$、$\boldsymbol\ell$、$\boldsymbol\alpha$、$\mathbf O^{\mathrm{acc}}$ |
| AIV1 | Query tile 后 64 个物理行的 V1/V2；没有有效输出行时跳过写回 | 与 AIV0 形状相同、内容独立的另一半状态 |

两路 AIV 不共享 Softmax 状态。它们在同一个 AIC 产生的矩阵 tile 上各取一半物理 Query 行，并分别生成自己的 `P` 分片。尾块可能让 AIV1 只包含少量有效输出行，甚至没有有效输出行；两路仍按同一同步协议完成固定物理 tile 的计算并归还槽位。v2～v11 把两份 `P` 写入共享 L1，v0/v1 则经 GM 交给 AIC；AIC 必须等两路 `P` 都准备好，才能执行固定大小的 C2。
...
| task | 某个 `(b,n)` 下一个 Query tile 的完整计算；它顺序处理所有允许读取的 Key/Value tile |
| slot | 一块可复用的物理片上缓冲；编号回卷时必须先确认上一位使用者已经释放 |
| group | v3～v5 中固定放在一起调度的两个 item |
| epoch | v6～v11 调度循环向前推进一次；同一次循环可以发射新阶段，也可以处理较早 item 的后续阶段 |
| $R$ | 首个 C2 前最多连续发射的 C1 数；首个 V2 前也最多连续发射相同数量的 V1 |

`R` 是代码中的调度参数，不是硬件自动预取次数。`R=3` 表示首个 C2 前最多先发射三个 C1，同一 item 的 C1 与 V2 相隔三次调度循环；相关的 `V/P/alpha` 也要准备三套轮换槽。增大 `R` 会占用更多 L1/UB；只有新增的 C1/V1 能填补原来的等待，耗时才会下降。
...
```bash
./build/Samples/2_Performance/flash_attn_lite_story/falite_v0 --size 1 1 385
./build/Samples/2_Performance/flash_attn_lite_story/falite_v11 --core-num 1 --size 1 1 768
```

| 参数 | 含义 |
| --- | --- |
| `--size S` | 只设置序列长度，`B=N=1`；`S` 可为任意正整数 |
| `--size N S` | 设置 Head 数和序列长度，`B=1` |
| `--size B N S` | 设置 Batch、Head 数和序列长度；不传时使用 `B=1,N=1,S=4096` |
...
```bash
FA_VERIFY_LOW_PRECISION_BASELINE=1 ./build/Samples/2_Performance/flash_attn_lite_story/falite_v9 --size 1 1 32768
```

v0～v11 均通过统一精度验证；`B=1,N=1,S=705` 的非整块用例也已逐版验通。各版本 README 给出覆盖流水填充、排空和槽位轮换的具体用例。

### 流水统计口径

| 数据 | 规格 | 用途 | 解读边界 |
| --- | --- | --- | --- |
| 长序列性能 | CANN 9.2、32 个 AIC、`B=1,N=1,S=131072` | 比较端到端 Kernel 时间和 MFU | 不含 Host Golden 时间 |
| 真机 PipeTimeline | CANN 9.2、1 组 Mix 核、`B=1,N=1,S=2048` | 观察核内 Pipe 的忙区、空隙和重叠 | 小规格时间不参与长序列性能比较 |
...
首个 C2 前发射的两个 C1 仍盖不住分步 Vector 的等待。v7 把这个数量增至三个，观察新增的 C1/V1 能否填入这些空隙。源码级流程见 [v6 README](src/v6/README.md)。

### v7：R=3，首个 C2 前发射三个 C1
...
截图窗口为 `[45.915, 85.915]` μs。首个 C2 前多发射一个 C1 后，CUBE、FIXP 和 VECTOR 形成更长的连续忙区，统一长序列测试的耗时较 v6 下降 22.19%。

这说明增加提前量确实能填补 v6 留下的等待。流水中仍能看到较长的 Vector 工作块，v8 让首个 C2 前再多发射一个 C1，检查单纯增加提前量还能否继续见效。源码级流程见 [v7 README](src/v7/README.md)。

### v8：R=4，再发射一个 C1，耗时基本不变
...
截图窗口为 `[44.312, 84.312]` μs。v7→v8 的统一长序列耗时只下降 0.08%，可视为相近水平。两版同时改变 `R` 和 L0C 槽数，因此这里只能评价整套配置：分步 Vector 没有缩短时，多提前发射一轮 C1/V1 没有带来可确认的端到端收益。

后续先压缩 V1/V2，再重新比较 `R=3/4/5`。源码级流程见 [v8 README](src/v8/README.md)。

## v9～v11：缩短 Vector 后重新比较 R

### 两条版本路线怎样对应

v7～v11 同时改变了“首个 C2 前发射多少个 C1”和“每个 Vector 阶段怎样计算”。下表把可直接比较的版本放在一起：
...
截图窗口为 `[39.165, 79.165]` μs。与 v8 相比，VECTOR 忙区变成更短的工作块；与 v9 相比，第四轮提前发射进一步填补较早阶段的等待。统一长序列数据中，v8→v10 耗时下降 29.77%，v9→v10 下降 19.79%。

L0C 的四个 FP32 tile 已占满 256 KiB，不能再随 `R` 增加。v11 保持 L0C 四槽，只把 `V/P/alpha` 的轮换槽增至五个。源码级流程见 [v10 README](src/v10/README.md)。

### v11：R=5，再增加一轮的收益很小

v11 允许首个 C2 前发射五个 C1，L0C 仍按四槽轮换。AIC L1 为第五份 `V/P` 增加空间，AIV 也保留第五份 `alpha`，但 C2 的 FP32 结果继续使用四槽 L0C。
...
这组结果说明，首个 C2/V2 前提前发射多少轮 C1/V1，要与 Vector 阶段的长度一起考虑。当 `R` 已经填满主要空隙后，继续增大它只会增加片上空间占用，耗时很难再明显下降。源码级流程见 [v11 README](src/v11/README.md)。
...
令 $n_{\mathrm{tile}}=\lceil S/128\rceil$。FALite 跳过所有未来 tile，但已发射的 C1/C2 仍按固定 $128\times128$ 计算，实际 tile 工作量为：

$$
F_{\mathrm{tile}}=2BND\times128^2\times
n_{\mathrm{tile}}(n_{\mathrm{tile}}+1).
$$
...
\frac{F_{\mathrm{effective}}}{t_{\mathrm{kernel}}\times432\times10^{12}},
$$

其中 $t_{\mathrm{kernel}}$ 是 Kernel Task Duration 的秒数，432 TFLOP/s 是 950PR 全部 32 个 AIC 的 BF16 Cube 峰值。统一规格下的工作量和峰值理论下限为：
...
| 版本 | 调度 | Vector 写法 | Task Duration（μs） | 有效吞吐（TFLOP/s） | 有效 Cube MFU | 高于有效下限（μs） | Tile Cube 利用率 |
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: |
...
- 支持 varlen、GQA/MQA 和更多 HeadDim，并补充完整网络需要的 bias、Dropout 或融合能力；
- 针对不足 128 行的尾块裁剪无效计算，减少固定物理 tile 带来的额外开销；
- 研究对角 tile 的专用计算，减少块内上三角仍由 Cube 执行的无效工作；
- 在保持精度标准的前提下继续压缩 Vector、数据搬运和跨核等待；
- 为不同序列长度、核数和片上空间约束选择合适的滚动距离，而不是固定追求更大的 `R`。

这个样例仍有继续优化的空间。欢迎开发者基于 [cann-samples 的 Flash Attention Lite 样例](https://gitcode.com/cann/cann-samples/tree/master/Samples/2_Performance/flash_attn_lite_story) 及各版本源码和流水证据，提出更简洁、更高效或覆盖能力更完整的实现。
