...
__aicore__ inline void CopyInB1(const AscendC::GlobalTensor<T>& bGlobal, const AscendC::LocalTensor<T>& bl1Local,
                                uint64_t curNL1, uint64_t curKL1, uint64_t k)
{
    AscendC::Nd2NzParams nd2nzParams;
    nd2nzParams.ndNum = 1;
...
__global__ __aicore__ void MatmulKernel(GM_ADDR aGm, GM_ADDR bGm, GM_ADDR cGm, uint32_t m, uint32_t k, uint32_t n)
{
    ...
    AscendC::GlobalTensor<hifloat8_t> aGlobal;
    AscendC::GlobalTensor<hifloat8_t> bGlobal;
    AscendC::GlobalTensor<float> cGlobal;
    aGlobal.SetGlobalBuffer(reinterpret_cast<__gm__ hifloat8_t*>(aGm));
    bGlobal.SetGlobalBuffer(reinterpret_cast<__gm__ hifloat8_t*>(bGm));
    cGlobal.SetGlobalBuffer(reinterpret_cast<__gm__ float*>(cGm));
    AscendC::LocalTensor<hifloat8_t> al0Local{AscendC::TPosition::A2, 0, L0A_SIZE};
    AscendC::LocalTensor<hifloat8_t> bl0Local{AscendC::TPosition::B2, 0, L0B_SIZE};
    AscendC::LocalTensor<float> l0cLocal{AscendC::TPosition::CO1, 0, L0C_SIZE};
    AscendC::LocalTensor<hifloat8_t> l1Local{AscendC::TPosition::A1, 0, L1_SIZE};

    constexpr static uint64_t HALF_L0_SIZE = L0A_SIZE / DOUBLE_BUFFER_COUNT / sizeof(hifloat8_t);

    // 设置基本块大小
    uint64_t baseM = 256;
    uint64_t baseN = 256;
    uint64_t baseK = 128 / sizeof(hifloat8_t);
    uint64_t kL1 = 512 / sizeof(hifloat8_t);
    ...
    for (uint64_t tileIdx = curBlockIdx; tileIdx < tileNum; tileIdx += blockNum) {
        ...
        for (uint64_t iter0 = 0; iter0 < kL1TileNum; ++iter0) {
            ...
            CopyInA1<hifloat8_t>(aGlobal[offsetA], l1Local[offsetAL1], mL1, curKL1, k);
            offsetA += curKL1;
            uint64_t offsetBL1 = baseM * kL1 * 2 + baseN * kL1 * l1BufId;
            CopyInB1<hifloat8_t>(bGlobal[offsetB], l1Local[offsetBL1], nL1, curKL1, k);
            offsetB += curKL1 * n;
            ...
            for (uint64_t iter1 = 0; iter1 < kL0TileNum; ++iter1) {
                ...
                CopyInA2<hifloat8_t>(al0Local[l0Offset], l1Local[offsetAL1], mL1, curKL1, mL0, curKL0);
                offsetAL1 += CeilAlign(mL1, AscendC::BLOCK_CUBE) * baseK;
                CopyInB2<hifloat8_t>(bl0Local[l0Offset], l1Local[offsetBL1], nL1, curKL1, nL0, curKL0);
                offsetBL1 += baseK * GetC0Size<hifloat8_t>();
...
int main(int argc, char* argv[])
{
    ...
    // 构造Host侧输入输出
    std::vector<uint8_t> hostInput(m * k, 0);
    std::vector<uint8_t> hostWeight(k * n, 0);
    std::vector<float> hostOutput(m * n, 0);
    std::vector<uint8_t> goldenOutput(m * n, 0);
    auto sizeInput = static_cast<size_t>(1) * hostInput.size() * sizeof(uint8_t);
    auto sizeWeight = static_cast<size_t>(1) * hostWeight.size() * sizeof(uint8_t);
    auto sizeOutput = static_cast<size_t>(1) * hostOutput.size() * sizeof(float);
    ReadFile("./input/input_x.bin", sizeInput, hostInput.data(), sizeInput);
    ReadFile("./input/input_weight.bin", sizeWeight, hostWeight.data(), sizeWeight);
    // 申请Device侧地址
    ...
    matmul::MatmulKernel<<<blockDim, nullptr, stream>>>(deviceInput, deviceWeight, deviceOutput, m, k, n);
