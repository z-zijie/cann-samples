...
// GetMxOffsetWithoutLayout
template <class BlockCoord, class ProblemShape>
__aicore__ inline AscendC::Coord<int64_t, int64_t, int64_t, int64_t, int64_t>
GetMxOffsetWithoutLayout(BlockCoord blockCoord, ProblemShape problemShape)
{
    int64_t m = Get<MNK_M>(problemShape);
    int64_t n = Get<MNK_N>(problemShape);
    int64_t k = Get<MNK_K>(problemShape);
    int64_t mOffset = Get<0>(blockCoord);
    int64_t nOffset = Get<1>(blockCoord);
    int64_t offsetA = mOffset * k;
    int64_t offsetB = nOffset * k;
    int64_t offsetC = mOffset * n + nOffset;
    int64_t offsetX1Scale = mOffset * CeilDiv(k, MXFP_DIVISOR_SIZE) * MXFP_MULTI_BASE_SIZE;
    int64_t offsetX2Scale = nOffset * CeilDiv(k, MXFP_DIVISOR_SIZE) * MXFP_MULTI_BASE_SIZE;
    return {offsetA, offsetB, offsetC, offsetX1Scale, offsetX2Scale};
}
