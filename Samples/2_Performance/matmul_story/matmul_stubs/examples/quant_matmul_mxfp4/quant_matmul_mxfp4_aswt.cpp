...
template <FullLoadMode FULL_LOAD_MODE>
    ...
    const QuantMatmulTilingData quantMatmulTilingData)
{
    ...
    using BlockScheduler = QuantMatmulMxAswtScheduler<static_cast<uint64_t>(FULL_LOAD_MODE)>;
    using DispatchPolicy = QuantMatmulMxMultiBlockWithAswt<AscendC::Shape<_0, _0, _0, _0>, static_cast<uint64_t>(FULL_LOAD_MODE)>;
...
int main(int argc, char* argv[])
{
    ...
    if (quantMatmulTilingData.enableAFullLoad) {
        QuantMatmulMxfp4Kernel<FullLoadMode::A_FULL_LOAD_MODE><<<quantMatmulTilingData.usedCoreNum, nullptr, stream>>>(
            dA, dB, dScaleA, dScaleB, dC, quantMatmulTilingData);
    } else {
        QuantMatmulMxfp4Kernel<FullLoadMode::NON_FULL_LOAD_MODE><<<quantMatmulTilingData.usedCoreNum, nullptr, stream>>>(
            dA, dB, dScaleA, dScaleB, dC, quantMatmulTilingData);
    }
