    ...
    int64_t n, int64_t k, int64_t cols)
{
    ...
    pipe_.InitBuffer(expandedRowIdxCopyInQueue_, PIPELINE_DEPTH, 
        ((curCorePerLoopIndicesElements_ * sizeof(int32_t) + 32 - 1) / 32));
    pipe_.InitBuffer(xCopyInQueue_, PIPELINE_DEPTH,
        ((cols * sizeof(T) + 32 - 1) / 32));
...
template <typename T>
void genInputData(size_t rows, size_t cols, std::vector<std::vector<T>>& res) {
...
template <typename T>
void printData(std::vector<std::vector<T>>& res) {
    for (auto& row : res) {
        for (auto& elem : row) {
...
int main()
{
    ...
    int64_t k = 1;
    int64_t h = 10;
    std::vector<std::vector<xType>> xData;
    genInputData(n, h, xData);
    printData(xData);

    std::vector<std::vector<indexType>> rowIdxData;
    genInputData(n, k, rowIdxData);
    printData(rowIdxData);

    std::vector<std::vector<xType>> expandedXData;
