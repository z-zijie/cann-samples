    ...
    GM_ADDR workspaceGM, MoeDistributeDispatchV2TilingData tilingData)
{
    // 待补齐kernel实现
    #ifndef REDUCED
        AscendC::TPipe pipe;
        MoeDistributeDispatchV2FullMeshImpl::MoeDistributeDispatchV2FullMesh<
            float16_t, float16_t, Mc2Kernel::UNQUANT, false, false> op;
        op.Init(x, expertIds, nullptr, nullptr, nullptr, nullptr, 
            ...
            workspaceGM, nullptr, &pipe, &tilingData);
        op.Process();
        return;
    #else
        AscendC::TPipe pipe;
...
int main(int argc, char* argv[])
{
    ...
    test_set_attr(rankId, rankNum, local_mem_size, ipport, default_flag_uid, &attributes);
    ...
    size_t localExpertNum = dispatchTilingData.moeDistributeDispatchV2Info.moeExpertNum
        / dispatchTilingData.moeDistributeDispatchV2Info.epWorldSize;
    size_t maxReceivedTokenNum = dispatchTilingData.moeDistributeDispatchV2Info.globalBs
        * std::min<size_t>(dispatchTilingData.moeDistributeDispatchV2Info.k, localExpertNum);
    ...
    // 待补齐相应参数生成和传递
    for (int i = 0; i < 1; ++i) {
        MoeDistributeDispatchKernel<<<48, nullptr, stream>>>(
