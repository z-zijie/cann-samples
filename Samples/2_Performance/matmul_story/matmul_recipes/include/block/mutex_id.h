 ...
 *
 * 各 block MMAD 头会被 block_mmad.h 聚合进同一编译单元，若各自在文件作用域重复
 * 定义这些 helper 会导致重定义错误，故统一抽取到此处（#pragma once 去重）。
 *
 * Mutex ID 分段约定：
 *   L1  (MTE2<->MTE1): [0, 8)
 *   L0  (MTE1<->M):    [8, 16)
 *   L0C (M<->FIX):     [16, 24)
 *   Scale(FIX<->MTE2): [24, 32)
 */
...
__aicore__ inline uint8_t ScaleMutex(uint16_t id) { return static_cast<uint8_t>(id + 24); }
} // namespace Block
