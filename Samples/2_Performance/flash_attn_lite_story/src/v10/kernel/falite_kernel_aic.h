    ...
    uint32_t kvTileCount)
{
    ...
    if ASCEND_IS_AIC {
        ...
        // 先取得 L0C，再等待 AIV 归还 UB 槽，防止后续 MMAD 覆盖尚未写出的结果。
            ...
            for (uint32_t epoch = 0; epoch < kvTileCount + CV_PIPELINE_SLOT_NUM; ++epoch) {
                if (epoch < kvTileCount) {
                    ...
                    CubeStage1(
                        ...
                        sSlotUBLocal, kGlobal, vGlobal, data, j, batchHeadIdx, kSlot, cvPipelineSlot, sSlot, l0abSlot,
                        l0cSlot, ioSlot, kvTileCount);
                    ++mmadOpIdx;
                }
