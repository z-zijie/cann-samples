    ...
    float softmaxScale, uint32_t requestedAicCoreNum, aclrtStream stream)
{
    ...
    const uint64_t pElements = static_cast<uint64_t>(data.numTasks) * data.br * data.bc;
    const uint64_t pBytes64 = pElements * sizeof(uint16_t);
    if (pBytes64 > std::numeric_limits<size_t>::max()) {
        std::fprintf(stderr, "kernel 未启动：P GM 中间缓冲区大小超出 size_t 表示范围\n");
        return false;
    }

    void* dP = nullptr;
    if (aclrtMalloc(&dP, static_cast<size_t>(pBytes64), ACL_MEM_MALLOC_HUGE_FIRST) != ACL_SUCCESS) {
        std::fprintf(stderr, "kernel 未启动：P GM 中间缓冲区分配失败\n");
        return false;
    }

    std::printf(
        "falite: 请求启动 kernel，AIC 核数=%u，scale=%g，GM workspace=%llu bytes\n", aicCoreNum,
        static_cast<double>(softmaxScale), static_cast<unsigned long long>(pBytes64));
    FALite::LaunchFlashAttnLiteKernel(dQ, dK, dV, static_cast<uint8_t*>(dP), dOut, data, stream);
