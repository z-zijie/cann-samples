...
public:
    ...
    {   
        ...
        DataCopyPadExtParams dataCopyPadParams{false, 0, 0, static_cast<DATA_TYPE>(0)};
        DataCopyPad(xInLocalTensor, xGm_[loop * tilingData_->r * tilingData_->ubFactor], dataCopyParams, dataCopyPadParams);
        ...
        __VEC_SCOPE__{
            ...
            for (uint16_t loopA = 0; loopA < static_cast<uint16_t>(ubFactor); loopA++) {
                ...
                for (uint16_t i = 0; i < vfLoopRNum; i++) {
                    ...
                    DataCopy<half, MicroAPI::LoadDist::DIST_UNPACK_B16>(vregXIn, xInAddr + loopA * rAlign_ + i * VL_FLOAT_SIZE);
        ...
        __VEC_SCOPE__{
            ...
            for (uint16_t loopA = 0; loopA < static_cast<uint16_t>(ubFactor); loopA++) {
                ...
                for (uint16_t i = 0; i < vfLoopRNum; i++) {
                    ...
                    MicroAPI::DataCopy<OUTPUT_DTYPE, MicroAPI::StoreDist::DIST_PACK4_B32>(yAddr + loopA * rAlign_ + i * VL_FLOAT_SIZE, VregY, preg);
                }
            }
        }
...
int64_t calcMaxUbFactor(size_t r) {
    ...
    for (; maxUbFactor < UB_SIZE; maxUbFactor++) {
        ...
        int64_t ubRemain = UB_SIZE - xInQueSize - gammaInQueSize - betaInQueSize - yOutQueSize - xBufSize - gammaBufSize - betaBufSize - rmsBufSize;
        if (ubRemain < 0) {
            break;
        }
    }
