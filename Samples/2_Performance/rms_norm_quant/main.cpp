...
void getDataFromBin(const std::string& filename, std::vector<T>& data) {
    ...
    // 检查文件大小是否为元素大小的整数倍
    if (remainder != 0) {
        std::cerr << "警告: 文件大小 (" << file_size << " 字节) 不是元素大小 (" 
                  << sizeof(T) << " 字节) 的整数倍" << std::endl;
        std::cerr << "将忽略最后 " << remainder << " 个字节的不完整数据" << std::endl;
    }
    ...
    if (num_elements > 0) {
        ...
        if (bytes_read != static_cast<std::streamsize>(num_elements * sizeof(T))) {
            std::cerr << "警告: 实际读取的字节数 (" << bytes_read 
                      << ") 与期望的字节数 (" << num_elements * sizeof(T) 
                      << ") 不符" << std::endl;
...
void genInputData(size_t totalSize, std::vector<T>& res) {
    ...
    for (int i = 0; i < totalSize; i++) {
        res[i] = float_to_half(static_cast<float>(i + 1));
    }
}
...
int32_t main(int argc, char* argv[]) {
    ...
    cmd << "python3 /mnt/workspace/gitCode/ops-samples-tpc/Samples/2_Performance/rms_norm_quant/gen_input_data.py "
        << "-r=" << r << " "
        << "-a=" << a << " "
        << "-d=float16";
    ...
    getDataFromBin("/mnt/workspace/gitCode/ops-samples-tpc/Samples/2_Performance/rms_norm_quant/input0.bin", xData);

    std::vector<dataType> gammaData;
    getDataFromBin("/mnt/workspace/gitCode/ops-samples-tpc/Samples/2_Performance/rms_norm_quant/input1.bin", gammaData);

    std::vector<dataType> betaData;
    getDataFromBin("/mnt/workspace/gitCode/ops-samples-tpc/Samples/2_Performance/rms_norm_quant/input2.bin", betaData);
    

    std::vector<scaleType> scaleData;
    getDataFromBin("/mnt/workspace/gitCode/ops-samples-tpc/Samples/2_Performance/rms_norm_quant/input3.bin", scaleData);

    std::vector<offsetType> offsetData;
    getDataFromBin("/mnt/workspace/gitCode/ops-samples-tpc/Samples/2_Performance/rms_norm_quant/input4.bin", offsetData);
    ...
    // 调用算子
    rms_norm_quant<dataType, scaleType, offsetType, outputType><<<blockNum, 0, stream>>>(xDevice, gammaDevice, betaDevice, scaleDevice, offsetDevice,  yDevice, tilingData);
    CHECK_ACL(aclrtSynchronizeStream(stream));
    CHECK_ACL(aclrtMemcpy(yHost, ySize, yDevice, ySize, ACL_MEMCPY_DEVICE_TO_HOST));

    std::vector<outputType> goldenData;
    getDataFromBin("/mnt/workspace/gitCode/ops-samples-tpc/Samples/2_Performance/rms_norm_quant/output0.bin", goldenData);
    ...
    for (int i = 0; i < yEleNum; i++) {
        if (abs(yHost[i] - goldenData[i]) > 1) {
            printf("Index: %04d RealIndex: %04d Expected: %3d Actual: %3d\n", errorDataIndex, i, static_cast<int>(goldenData[i]), static_cast<int>(yHost[i]));
            errorDataIndex++;
        }
    }
