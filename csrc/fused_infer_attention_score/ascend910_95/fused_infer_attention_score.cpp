...
#include "op_kernel/fia_demo_entry_regbase.h"
namespace x {
namespace FIA {

__global__ __aicore__ void fia_kernel(
    GM_ADDR query, GM_ADDR key,
    GM_ADDR value, GM_ADDR attenMask,
    GM_ADDR attentionOut, GM_ADDR workspace,
    GM_ADDR tiling
    )
{
    KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_MIX_AIC_1_2);
    prompt_flash_attention_FIAS_regbase<0, 3, true>(
        query, key, value,
...
at::Tensor fia_npu( const at::Tensor& q,                    // 查询张量 [B, Lq, H, D]
    ...
    // workspace
    int64_t workspaceSize =  workspaceSize = 200 * 2048 * 1024; // default value
    ...
    FiaDemoCompileInfo tempCompileInfoPtr;
    ...
    FiaDemoTiling tiling;
    auto ret = tiling.RunBigKernelTilingWithParams(contextParamsForTiling);
    ...
    auto acl_call = [=]() -> int {
        fia_kernel<<<blockDimToBeSet, nullptr, aclstream>>>(
    ...
    at_npu::native::OpCommand::RunOpApiV2("FIA", acl_call);
...
torch::Tensor fia_meta( const torch::Tensor& q,                // 查询张量 [B, Lq, H, D]
...
{
    m.impl("fused_infer_attention_score", fia_npu);
}

// Register Meta Function for RopeMatrix
TORCH_LIBRARY_IMPL(EXTENSION_MODULE_NAME, Meta, m)
{
    m.impl("fused_infer_attention_score", TORCH_FN(fia_meta));
}
