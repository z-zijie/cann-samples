...
constexpr int64_t UB_BLOCK_SIZE = 32; // 32byte对齐
...
{
    // Tiling4VBSCompute
    int64_t sortLoopMaxElement = tilingData->ubSize / (4 * 2 * 4) / ONE_REPEAT_SORT_NUM * ONE_REPEAT_SORT_NUM;
    sortLoopMaxElement =
...
int main()
{
    ...
    size_t workspaceSize = Align(n * k, sizeof(int32_t)) * 2 + 
                           Align(actualExpertNum_, sizeof(int32_t)) + 
