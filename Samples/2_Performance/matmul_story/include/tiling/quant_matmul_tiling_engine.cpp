...
void QuantMatmulTilingEngine::CalL1Tiling()
{
    ...
    runInfo_.l0cDB = runInfo_.baseM * runInfo_.baseN * DATA_SIZE_L0C * DB_SIZE <= platformInfo_.l0cSize ? DB_SIZE : 1U;
    if (runInfo_.isAFullLoad) {
    ...
    uint64_t baseASize, uint64_t baseBSize, uint64_t baseScaleASize, uint64_t baseScaleBSize)
{
    ...
    uint64_t scaleFactorA = inputParams_.kSize / (runInfo_.stepKa * runInfo_.baseK);
    uint64_t scaleFactorB = inputParams_.kSize / (runInfo_.stepKb * runInfo_.baseK);
...
{
    tilingData.adaptiveSlidingWin.mTailTile = adaptiveWin_.mTailTile;
    tilingData.adaptiveSlidingWin.nTailTile = adaptiveWin_.nTailTile;
    tilingData.adaptiveSlidingWin.mBaseTailSplitCnt = adaptiveWin_.mBaseTailSplitCnt;
    tilingData.adaptiveSlidingWin.nBaseTailSplitCnt = adaptiveWin_.nBaseTailSplitCnt;
    tilingData.adaptiveSlidingWin.mTailMain = adaptiveWin_.mTailMain;
    tilingData.adaptiveSlidingWin.nTailMain = adaptiveWin_.nTailMain;

    tilingData.matmulTiling.m = inputParams_.mSize;
    tilingData.matmulTiling.n = inputParams_.nSize;
    tilingData.matmulTiling.k = inputParams_.kSize;

    tilingData.matmulTiling.baseM = basicTiling_.baseM;
    tilingData.matmulTiling.baseN = basicTiling_.baseN;
    tilingData.matmulTiling.baseK = basicTiling_.baseK;
    tilingData.matmulTiling.dbL0C = basicTiling_.dbL0c;

    tilingData.matmulTiling.scaleKL1 = std::min(
...
{
    tilingData.matmulTiling.stepKa = std::min(basicTiling_.stepKa, basicTiling_.stepKb);
    tilingData.matmulTiling.stepKb = tilingData.matmulTiling.stepKa;
    uint64_t kL1 = tilingData.matmulTiling.stepKa * tilingData.matmulTiling.baseK;
    uint64_t usedL1Size = GetSizeWithDataTypeFP4(basicTiling_.baseN * kL1) * L1_FOUR_BUFFER;
    usedL1Size += basicTiling_.baseN * CeilDiv(tilingData.matmulTiling.scaleKL1, MX_GROUP_SIZE) * DB_SIZE;
    if (inputParams_.hasBias) {
        usedL1Size += GetSizeWithDataTypeFP4(basicTiling_.baseN, inputParams_.biasDtype) * DB_SIZE;
    }
    if (isAFullLoad_) {
        uint64_t scaleK = CeilDiv(inputParams_.kSize, MXFP_DIVISOR_SIZE) * MXFP_MULTI_BASE_SIZE;
        uint64_t kAligned = CeilAlign(inputParams_.kSize, MXFP_DIVISOR_SIZE);
        usedL1Size += GetSizeWithDataTypeFP4(basicTiling_.baseM * kAligned) + basicTiling_.baseM * scaleK;
    } else {
        usedL1Size += GetSizeWithDataTypeFP4(basicTiling_.baseM * kL1) * L1_FOUR_BUFFER;
        usedL1Size += basicTiling_.baseM * CeilDiv(tilingData.matmulTiling.scaleKL1, MX_GROUP_SIZE) * DB_SIZE;
    }
    tilingData.matmulTiling.nBufferNum = usedL1Size < aicoreParams_.l1Size ? L1_FOUR_BUFFER : DB_SIZE;
}
