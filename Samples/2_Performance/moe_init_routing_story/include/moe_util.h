...
template <typename T>
void writeDataToBin(const std::string& filename, std::vector<T>& data) {
    // 空数据检查
    if (data.empty()) {
        throw std::invalid_argument("错误: 待写入的vector为空");
    }
    ...
    try {
        ...
        outfile.write(reinterpret_cast<const char*>(data.data()), total_bytes);
