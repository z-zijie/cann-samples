...
public:
    ...
    __aicore__ inline void SortOutProcess()
    {
        if (this->blockIdx_ < 1) {
            ...
            sorter.Init(&mrgsortParam, pipe_);
            sorter.Process();
        }
        SyncAll();
    }

    __aicore__ inline void OneCoreProcess()
    {
        if (this->blockIdx_ < 1) {
            CopyIn();
            SortCompute();
    ...
    {
        if (this->totalLength < this->vbsTilingData->oneLoopMaxElements) {
            OneCoreProcess();
            return;
        }
        ...
        SortOutProcess();
        if (this->blockIdx_ < 1) {
            DumpTensor(sortedexpertIdxGm, 100, 8);
            DumpTensor(expandedRowIdxGm, 100, 32);
        }
    }
};
