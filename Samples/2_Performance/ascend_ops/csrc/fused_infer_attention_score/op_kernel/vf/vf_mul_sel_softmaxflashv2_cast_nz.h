...
template <typename T, typename T2, typename pseShiftType, uint32_t s1BaseSize = 64, uint32_t s2BaseSize = 256,
    bool hasAtten = 0, bool hasDrop = 0>
...
template <typename T, typename T2, typename pseShiftType, uint32_t s1BaseSize = 128, uint32_t s2BaseSize = 128,
    bool hasAtten = 0, bool hasDrop = 0, bool isMlaSgd = false>
...
__aicore__ inline void ProcessVec1NoUpdateImpl128(
    ...
    const uint32_t originN, const float slopes, const float posShift, const T scale, const float dScaleQK,
    ...
    const T minValue, float keepProb)
{
    if constexpr (oriNRange == GT_128_AND_LTE_256) {
        ProcessVec1NoUpdateGeneralImpl256<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, hasDrop>(dstTensor,
            expSumTensor, maxTensor, srcTensor, expMaxTensor, inExpSumTensor, inMaxTensor, maskTensor, pseTensor,
            dropTensor, sharedTmpBuffer, m, originN, slopes, posShift, scale, minValue, keepProb);
    } else if constexpr (oriNRange == EQ_128) {
       
        LocalTensor<uint8_t> indexesTensor;
        ProcessVec1NoUpdateImpl128<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, hasDrop, isMlaSgd>(dstTensor, indexesTensor, expSumTensor,
            maxTensor, srcTensor, expMaxTensor, inExpSumTensor, inMaxTensor, maskTensor, pseTensor, dropTensor,
            sharedTmpBuffer, m, originN, slopes, posShift, scale, dScaleQK, minValue, keepProb);
    } else if constexpr (oriNRange == GT_0_AND_LTE_64) {
        LocalTensor<uint8_t> indexesTensor;
        ProcessVec1NoUpdateImpl64<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, hasDrop, isMlaSgd>(dstTensor, indexesTensor, expSumTensor,
            maxTensor, srcTensor, expMaxTensor, inExpSumTensor, inMaxTensor, maskTensor, pseTensor, dropTensor,
            sharedTmpBuffer, m, originN, slopes, posShift, scale, dScaleQK, minValue, keepProb);
    } else if constexpr (oriNRange == GT_256_AND_LTE_512) {
        ProcessVec1NoUpdateGeneralImpl512<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, hasDrop>(dstTensor, expSumTensor,
            maxTensor, srcTensor, expMaxTensor, inExpSumTensor, inMaxTensor, maskTensor, pseTensor, dropTensor,
            sharedTmpBuffer, m, originN, slopes, posShift, scale, minValue, keepProb);
    } else if constexpr (oriNRange == GT_512_AND_LTE_1024) {
        ProcessVec1NoUpdateGeneralImpl1024<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, hasDrop>(dstTensor,
            expSumTensor, maxTensor, srcTensor, expMaxTensor, inExpSumTensor, inMaxTensor, maskTensor, pseTensor,
            dropTensor, sharedTmpBuffer, m, originN, slopes, posShift, scale, minValue, keepProb);
    } else { // GT_64_AND_LTE_128
        LocalTensor<uint8_t> indexesTensor;
        ProcessVec1NoUpdateGeneralImpl128<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, hasDrop, isMlaSgd>(dstTensor, indexesTensor, 
            expSumTensor, maxTensor, srcTensor, expMaxTensor, inExpSumTensor, inMaxTensor, maskTensor, pseTensor,
            dropTensor, sharedTmpBuffer, m, originN, slopes, posShift, scale, dScaleQK, minValue, keepProb);
    }
}
...
__aicore__ inline void ProcessVec1UpdateGeneralImpl256(
    ...
    const LocalTensor<T>& inMaxTensor, const LocalTensor<uint8_t>& maskTensor, const LocalTensor<pseShiftType>& pseTensor,
    const LocalTensor<uint8_t>& dropTensor, const LocalTensor<uint8_t>& sharedTmpBuffer, const uint16_t m,
    const uint32_t originN, const float slopes, const float posShift, const T scale,
    const T minValue, float keepProb)
{
    constexpr uint32_t nPadding = (s2BaseSize + blockBytesU8 - 1) / blockBytesU8 * blockBytesU8;
    // 写的时候固定用65或者33的stride去写，因为正向目前使能settail之后mm2的s1方向必须算满128或者64行
...
{
    if constexpr (oriNRange == GT_128_AND_LTE_256) {
        ProcessVec1UpdateGeneralImpl256<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, hasDrop>(
            ...
            slopes, posShift, scale, minValue, keepProb);
    } else if constexpr (oriNRange == EQ_128) {
        LocalTensor<uint8_t> indexesTensor;
        ProcessVec1UpdateImpl128<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, hasDrop, isMlaSgd>(
            dstTensor, indexesTensor, expSumTensor, maxTensor, srcTensor, expMaxTensor,
            inExpSumTensor, inMaxTensor, maskTensor, pseTensor, dropTensor, sharedTmpBuffer, m, originN,
            slopes, posShift, scale, dScaleQK, minValue, keepProb);
    } else if constexpr (oriNRange == GT_0_AND_LTE_64) {
        LocalTensor<uint8_t> indexesTensor;
        ProcessVec1UpdateImpl64<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, hasDrop, isMlaSgd>(
            dstTensor, indexesTensor, expSumTensor, maxTensor, srcTensor, expMaxTensor,
            inExpSumTensor, inMaxTensor, maskTensor, pseTensor, dropTensor, sharedTmpBuffer, m, originN,
            slopes, posShift, scale, dScaleQK, minValue, keepProb);
    } else if constexpr (oriNRange == GT_256_AND_LTE_512) {
        ProcessVec1UpdateGeneralImpl512<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, hasDrop>(
            dstTensor, expSumTensor, maxTensor, srcTensor, expMaxTensor,
            inExpSumTensor, inMaxTensor, maskTensor, pseTensor, dropTensor, sharedTmpBuffer, m, originN,
            slopes, posShift, scale, minValue, keepProb);
    } else if constexpr (oriNRange == GT_512_AND_LTE_1024) {
        ProcessVec1UpdateGeneralImpl1024<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, hasDrop>(
            dstTensor, expSumTensor, maxTensor, srcTensor, expMaxTensor,
            inExpSumTensor, inMaxTensor, maskTensor, pseTensor, dropTensor, sharedTmpBuffer, m, originN,
            slopes, posShift, scale, minValue, keepProb);
    } else { // GT_64_AND_LTE_128
        LocalTensor<uint8_t> indexesTensor;
        ProcessVec1UpdateGeneralImpl128<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, hasDrop, isMlaSgd>(
            dstTensor, indexesTensor, expSumTensor, maxTensor, srcTensor, expMaxTensor,
            inExpSumTensor, inMaxTensor, maskTensor, pseTensor, dropTensor, sharedTmpBuffer, m, originN,
            slopes, posShift, scale, dScaleQK, minValue, keepProb);
    }
...
template <typename T, typename T2, typename pseShiftType, bool isUpdate = false, uint32_t s1BaseSize = 128, uint32_t s2BaseSize = 128,
          OriginNRange oriNRange = GT_64_AND_LTE_128, bool hasAtten = 0,
           bool hasDrop = 0, bool isMlaSgd = false>
    ...
    const float slopes, const float posShift, const T scale, const float dScaleQK, const T minValue, float keepProb)
{
    ...
    if constexpr (!isUpdate) {
        ProcessVec1NoUpdate<T, T2, pseShiftType, s1BaseSize, s2BaseSize, oriNRange, hasAtten, hasDrop, isMlaSgd>(
            ...
            m, originN, slopes, posShift, scale, dScaleQK, minValue, keepProb);
    } else {
        ProcessVec1Update<T, T2, pseShiftType, s1BaseSize, s2BaseSize, oriNRange, hasAtten, hasDrop, isMlaSgd>(
            dstTensor, vselrIndexesBuf, expSumTensor, maxTensor, srcTensor, expMaxTensor,
            inExpSumTensor, inMaxTensor, maskTensor, pseTensor, dropTensor, sharedTmpBuffer,
            m, originN, slopes, posShift, scale, dScaleQK, minValue, keepProb);
    }
