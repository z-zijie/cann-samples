...
// 计算 OAcc = alpha * OAcc + oDelta. OAcc 初始为 0, 无需单独处理首轮.
// alpha 通过 BRC 按行广播.
...
// V1：分步更新 Softmax 状态，再将未归一化权重 Pᵀ 从 FP32 DN 转为 BF16 NZ。
    ...
    const FlashAttnLiteTilingData& data, uint32_t j, uint32_t qTileIdx, uint32_t subAivIdx)
{
    ...
    if ASCEND_IS_AIV {
        ...
        asc_vf_call<FusedDNToNZCastVF>(
            ...
            reinterpret_cast<__ubuf__ float*>(sUBLocal.GetPhyAddr()), halfBr, bc);
    }
}
...
// V2: 使用通用路径更新 OAcc = alpha * OAcc + oDelta.
...
__aicore__ inline void KernelProcessForAIV(__gm__ bfloat16_t* outGMAddr, FlashAttnLiteTilingData data)
{
    ...
    if ASCEND_IS_AIV {
        ...
        for (uint32_t taskId = aicIdx; taskId < data.numTasks; taskId += data.useAicNum) {
            ...
            // m/l 不占用输出槽，可与上一 task 的 MTE3 写回并行。
            ...
            // 分步 Vector 路径在每个 task 开始时将递推状态初始化为 0。
            ...
            if (outputRows > 0) {
                ...
                DataCopy(outGlobal[outGMOffset], outputSlotUBLocal, outputRows * d);
