...
def verify_result(m, n):
    with open("./output/npu_out.bin", "rb") as f1:
        output_data = f1.read()
    with open("./output/golden_output.bin", "rb") as f2:
        golden_data = f2.read()
    output = np.frombuffer(output_data, dtype=np.uint8)
    golden = np.frombuffer(golden_data, dtype=np.uint8)
